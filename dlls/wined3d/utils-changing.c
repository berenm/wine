...
static void dxtn_decompress_block(const BYTE *src, BYTE *dst, UINT width, UINT height, UINT depth,
        UINT x_pos, UINT y_pos, UINT z_pos, UINT dst_slice_pitch, UINT64 cur_block, BYTE fmt)
{
        ...
        UINT dst_row_pitch, UINT dst_slice_pitch, UINT width, UINT height, UINT depth)
{
    ...
    for (z = 0; z < depth; ++z)
    {
        for (y = 0; y < height; y += 4)
        {
            for (x = 0; x < width; x += 4)
            {
                dxtn_decompress_block(src, dst, width, height, depth, x, y, z, dst_slice_pitch, current_block, WINED3DFMT_DXT5);
                current_block++;
            }
        }
    }
